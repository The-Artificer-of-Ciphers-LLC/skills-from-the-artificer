...
> **🛑 STRICT EXECUTION PROTOCOL (SEQUENCE ENFORCEMENT & MEMORY):** You are operating within Open Code. You MUST execute these steps in strict, numerical sequence. Do NOT skip ahead, and do NOT jump to Step 6 (PR submission) until Steps 1 through 5 are entirely completed and logged into memory.
>
...
**[BLOCKING]** *Do not modify any code until this step is complete and logged.*
...
3. Use file search and viewing tools, alongside `get_symbol_context` from Memtrace, to read all relevant internal source code files comprehensively.
...
6. Verify cross-compatibility with the existing codebase before modifying or creating files using the `change-impact-analysis` workflow.

## Step 2: Rubber-Duck Design & Software Law Compliance
**[BLOCKING]** *Architecture must be verified and logged before testing.*
...
3. Validate your proposed changes using the `get_impact` skill to determine the blast radius and risk rating of the new design.

## Step 3: QA Architecture & Test-Driven Implementation
**[BLOCKING]** *Tests must pass using `gsd-test` before review. Log completion when done.*
...
4. Run `detect_changes` to verify the exact diff scope and ensure no unintended symbol references or unapproved structural changes leak into the commit.
...
3. Run `continuous-memory` one final time to finalize the state of the session before the PR.

## Step 6: Dynamic Next-Branch Rebase & Immediate PR Submission
**[FINAL STEP]** *Execute only after Steps 0-5 are completely resolved and sequentially logged in memory.*
